...
A modern, well-structured React web application providing a suite of AI-powered services, secure file storage, and user authentication. Bootstrapped with [Create React App](https://github.com/facebook/create-react-app).
...
```
├── public/               # Static assets (index.html, favicon, manifest, robots.txt, images, videos)
├── src/
│   ├── components/       # Reusable UI components (Sidebar, Footer, CallbackCleaner, NotificationContext, AdminNotifications)
│   ├── pages/            # Main feature & marketing pages (Landing, About, Features, Testimonials, Pricing, Contact, Login, Dashboard, AI services, Notifications, AdminPushMessage)
│   ├── services/         # API and SSE logic (notificationService.js)
│   ├── config/           # Branding, notification, subscription settings
...
├── build/                # Production build output (git-ignored except in production branch)
...
```bash
git clone https://github.com/Annadaneshwar1998/Servantica.in.git
cd Servantica.in
npm install
```
...
```
REACT_APP_FACE_KEY=your_azure_face_api_key
REACT_APP_SPEECH_KEY=your_azure_speech_api_key
REACT_APP_CV_KEY=your_azure_computervision_api_key
REACT_APP_AUTH0_DOMAIN=your_auth0_domain
REACT_APP_AUTH0_CLIENT_ID=your_auth0_client_id
REACT_APP_KEYCLOAK_BASE_URL=https://keycloak-app.whitebush-8de3c8f4.centralindia.azurecontainerapps.io
REACT_APP_KEYCLOAK_REALM=servantica
REACT_APP_KEYCLOAK_CLIENT_ID=servantica
REACT_APP_ENABLE_AI_SERVICES=true
REACT_APP_ENABLE_FILES=true
```
...
This project uses a GitHub Actions workflow for automated deployment:

- **Trigger:** On every push to the `main` branch.
- **Process:**
  - Installs dependencies and builds the React app.
  - **Deploys only the contents of the `build` folder** to the root of the `production` branch.
  - Cleans out any previous files in the `production` branch before copying the new build.
- **Workflow file:** `.github/workflows/deploy.yml`

> The `production` branch will always contain a clean, up-to-date build ready for static hosting.

---

## Subscription & Gating Cheatsheet

- Config lives in `src/config/subscriptions.js`:
  - `enabled`: hide/show a feature (sms, call, ai).
  - `requiresSubscription`: whether unlock is needed.
  - `priceLabel`: label shown in sidebar/Pricing add-ons.
- Sidebar and Pricing read from this config, so edits stay consistent.
- AI can also be globally toggled with `REACT_APP_ENABLE_AI_SERVICES`.

## Auth & Tokens (Auth0 + Keycloak)

- Auth paths: Login page offers Google via Auth0 and Keycloak login/sign-up flows.
- Keycloak tokens: On callback, the app exchanges the `code` for `access_token`/`refresh_token`/`id_token` and stores them in `localStorage`. These are used first for API calls.
- Auth0 tokens (machine-to-machine):
  - `src/services/machineTokenService.js` handles Auth0 client-credentials with caching.
  - `src/services/notificationApi.js` (SMS) and `src/services/callApi.js` use these as fallback when no user token is present.
- User token preference: `src/utils/authToken.js` picks the best available user token (Keycloak access token preferred; Auth0 user token if stored) for outbound API calls; falls back to machine tokens if absent.
...
```bash
git clone https://github.com/Annadaneshwar1998/Servantica-AI.git
cd Servantica-AI
```
...
openai.api.video-url=https://api.openai.com/v1/videos

# Servantica Microservices
servantica.notification-service.base-url=https://annadaneshwar-notification-service.azurewebsites.net/
servantica.call-service.base-url=https://annadaneshwar-cms.azurewebsites.net/
servantica.service.auth.username=Anand
servantica.service.auth.password=Anand@8676
```
...
| URL | Description |
|---|---|
| `http://localhost:8080/swagger-ui/index.html` | Swagger UI — all APIs |
| `http://localhost:8080/assistant.html` | 🤖 Servantica Assistant UI (text + voice) |

---

## 🔐 Authentication

Three methods supported — the system auto-detects per request:
...
curl -X POST http://localhost:8080/api/assistant/ask \
  ...
  -d '"Send SMS to +917259510747 with message Hello"'
```
...
```bash
# Get token
curl -X POST 'https://keycloak-app.whitebush-8de3c8f4.centralindia.azurecontainerapps.io/realms/servantica/protocol/openid-connect/token' \
  -d 'client_id=servantica&username=Anand&password=Anand@12&grant_type=password'

# Use token
curl -X POST http://localhost:8080/api/assistant/ask \
  -H "Authorization: Bearer <KEYCLOAK_JWT>" \
  -H "Content-Type: application/json" \
  -d '"What APIs are available?"'
```
...
> Full details: [Authentication Guide](docs/authentication.md)
...
|---|---|---|
| `POST` | `/api/chatbot` | Get ChatGPT response |

### 🎨 Image Generation API

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/image/generate` | Generate image → downloadable PNG |
...
| `GET` | `/api/video/retrieve/{videoId}` | Check video generation status |
| `GET` | `/api/video/download/{videoId}` | Download completed video (MP4) |

### 🤖 Servantica Assistant API

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/assistant/ask` | Ask a question or execute an action |
| `POST` | `/api/assistant/chat/{sessionId}` | Multi-turn conversation (session-based) |
| `DELETE` | `/api/assistant/chat/{sessionId}` | Clear a conversation session |
| `GET` | `/api/assistant/sessions` | List active sessions |
| `GET` | `/api/assistant/topics` | Get capabilities & help topics |

### 🎤 Voice Assistant API

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/assistant/voice` | Single voice command (audio upload → voice response) |
| `POST` | `/api/assistant/voice/chat/{sessionId}` | Multi-turn voice conversation |
| `POST` | `/api/assistant/tts` | Text-to-Speech — convert text to MP3 |
| `POST` | `/api/assistant/stt` | Speech-to-Text — transcribe audio to text |

### 🎙️ Voice Cloning API (ElevenLabs)

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/assistant/voice/clone` | Upload audio samples → clone voice (returns voiceId) |
| `POST` | `/api/assistant/voice/cloned-tts` | Generate TTS with cloned voice (returns MP3) |
| `GET` | `/api/assistant/voice/cloned-voices` | List all available ElevenLabs voices |

### 🚨 Emergency Protocol API

| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/api/assistant/emergency` | Trigger emergency — sends GPS SMS + auto-call to emergency contact |

### 📄 Document API

| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/api/docs/readme` | Download README as styled PDF |
| `GET` | `/api/docs/download` | Download full docs as ZIP |
...
| `/voice-assistant` | 🤖 Servantica Assistant UI (text + voice + wake word) |
...
```
Servantica-AI/
├── src/main/java/com/example/Servantica/AI/
│   ├── ServanticaAiApplication.java            # Main entry point
│   ├── Config/
│   │   ├── SecurityConfig.java                  # Auth0 + Keycloak + Basic Auth
│   │   └── SwaggerConfig.java                   # OpenAPI/Swagger configuration
│   ├── Controller/
│   │   ├── ChatbotController.java               # ChatGPT API
│   │   ├── ImageController.java                 # DALL·E image generation
│   │   ├── VideoController.java                 # Sora-2 video generation
│   │   ├── ServanticaAssistantController.java   # AI assistant + voice + actions
│   │   └── DocumentController.java              # PDF/ZIP doc downloads
│   └── Service/
│       ├── VoiceService.java                    # Whisper STT + OpenAI TTS
│       └── ElevenLabsVoiceService.java          # ElevenLabs voice cloning & TTS
├── src/main/resources/
│   ├── application.properties                   # App configuration
│   └── static/
│       └── assistant.html                       # 🤖 Servantica Assistant UI v2
├── docs/
...
| Service | URL |
|---|---|
| **Servantica AI** | `https://annadaneshwar-ai.azurewebsites.net/` |
| **Swagger UI** | `https://annadaneshwar-ai.azurewebsites.net/swagger-ui/index.html` |
| **Voice Assistant** | `https://annadaneshwar-ai.azurewebsites.net/voice-assistant` |
| **Notification Service** | `https://annadaneshwar-notification-service.azurewebsites.net/` |
| **Call Management** | `https://annadaneshwar-cms.azurewebsites.net/` |

CI/CD via GitHub Actions on push to `main`. See [Deployment Guide](docs/deployment.md).
